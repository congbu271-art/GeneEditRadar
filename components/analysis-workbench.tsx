...
export { getDefaultResultTab, getResultSectionSequence } from "@/lib/analyze-ui-helpers";
...
const EmptyResultState = memo(function EmptyResultState() {
  return (
...
export function AnalysisWorkbench() {
  ...
  const structuredFeatureMap = useMemo(
    () => new Map((result?.structuredFeatures ?? []).map((feature) => [feature.paperId, feature])),
    [result],
  );
  const workflowPhase = isLoading ? "loading" : result ? "complete" : "idle";
  const overviewTools = useMemo(
    () => (result ? collectTopFeatureValues(result.structuredFeatures, "editingTool") : []),
    [result],
  );
  const overviewOrganisms = useMemo(
    () => (result ? collectTopFeatureValues(result.structuredFeatures, "organism") : []),
    [result],
  );
